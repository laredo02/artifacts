...
public class Main extends Application {
	...
	public void start (Stage stage) {
		...
		stlMeshImporter.read("Bike.stl"); 
		TriangleMesh mesh = stlMeshImporter.getImport();
		stlMeshImporter.close();
		MeshView stl = new MeshView(mesh);
		stl.setMaterial(new PhongMaterial(Color.rgb(100, 100, 200)));
		stl.setScaleX(10);
		stl.setScaleY(10);
		stl.setScaleZ(10);
		stl.setTranslateX(-300);
		stl.setTranslateY(0);
		stl.setTranslateZ(3000);
		stl.getTransforms().addAll(new Rotate(90, Rotate.Z_AXIS));
		stl.setTranslateY(-50);

		rootNode.getChildren().addAll(box, stl, ground, b, sphere, head);
		...
		if (videoCapture.isOpened()) {
			...
			while (running) {
				...
				if (!mat.empty()) {
					...
					CascadeClassifier cascadeClassifier = new CascadeClassifier("/usr/local/share/opencv4/haarcascades/haarcascade_frontalcatface.xml");
